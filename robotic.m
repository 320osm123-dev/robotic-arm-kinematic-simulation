...
%% ================== 초기 Forward ==================
[ok, pts2d] = forward_from_beta_t2_2d( ...
    beta, t2, P8, P7, ...
    L84, L45, L24, ANG_46_45, L12, L13, rP2, rP4);

if ~ok
    error('초기 자세(beta,t2)에서 해가 안 나옵니다. beta/t2 초기값을 바꿔야 합니다.');
end
...
if ~isfinite(beta) || ~isfinite(t2) || abs(beta)>720 || abs(t2)>720
    J = 1e9; return;
end

[ok, pts] = forward_from_beta_t2_2d(beta, t2, ...
    P8, P7, L84, L45, L24, ANG_46_45, L12, L13, rP2, rP4);
...
e_reg = abs(beta - x_prev(1)) + 0.7*abs(t2 - x_prev(2));

J = w_xy*e_xy^2 + w_tri*e_tri^2 + w_reg*e_reg^2;
end

%% ================== Forward ==================
function [ok, pts] = forward_from_beta_t2_2d( ...
    beta, t2, P8, P7, L84, L45, L24, ANG_46_45, L12, L13, rP2, rP4)
